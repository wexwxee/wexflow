...
def clear_offers() -> int:
    """УСТАРЕЛО: больше нигде не вызывается (июль 2026). Сброс «предложено»
    после смены фильтров приводил к повторной отправке одних и тех же
    вакансий в Telegram и нарушал гейт F27 «предложено — навсегда».
    Оставлено только как ручной инструмент на крайний случай."""
    removed = 0
    with get_session() as s:
        rows = s.exec(select(Application).where(
