...
def barrier_clause():
    """Условие SQL «языковой барьер не ДОКАЗАН» (или None — не фильтруем).

    NULL (ещё не оценивали) и «не ясно» проходят: молчание оценщика не повод
    прятать вакансию.

    С 11.08.2026 проходит и догадка ИИ: вердикт `danish`/`diploma` прячет
    вакансию только тогда, когда он поставлен ПРАВИЛАМИ, то есть по цитате из
    самого объявления. Мнение модели о рядовой работе остаётся в ленте с
    пометкой — см. `relevance.is_barrier`, там же вся история вопроса.

    Второй случай сокрытия — руководящая должность при вердикте ИИ — в SQL не
    выразить (это регулярка по названию), он досчитывается в Python тем же
    `relevance.is_barrier`. Здесь условие намеренно мягче: ошибаться лучше в
    сторону «показать», а автопилот руководящие отсеивает сам.
    """
    if not hide_barrier():
        return None
    import relevance
    return (
        Job.fit.is_(None)
        | Job.fit.not_in(list(relevance.BARRIER))
        | Job.fit_engine.ilike("ai%")
    )
